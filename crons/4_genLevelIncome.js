  ...
  async function incomeCal() {
    ...
    test.forEach(async (element) => {
      ...
      switch (true) {
        ...
        default:
          console.log(
            `no condition match memberId:${memberId},level:${element.level},teammember:${team},directMember:${directMember},`
          );
          break;
      }
    });
...
async function getTotalLevel() {
  let str = "SELECT count(*) FROM tbl_memberreg where status = 1";
